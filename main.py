...
한투 API 거래량 보통주 전체 → 네이버 차트 → 단테 스코어링
"""
import sys, time
...
def scan_dante(candidates, market_name):
    ...
    for i, stock in enumerate(candidates, 1):
        ...
        try:
            ta = calc_technical(stock['code'], days=500)
            if not ta:
                continue
            ...
            if score['mandatory'] >= 3:
                results.append({
                    ...
                    'price': ta['price'], 'change_pct': stock.get('change_pct', 0),
                    'volume': stock.get('volume', 0), 'score': score, 'ta': ta,
                })
            time.sleep(0.15)
        except:
            pass
...
def format_portfolio(data):
    ...
    for p in data:
        ...
        lines.append(f"{emoji} {p['name']}: {p['cur_price']:,}원 ({p['change_pct']:+.2f}%) | 수익률 {p['pnl_pct']:+.1f}%")
    if total_invested > 0:
...
def format_dante_top3(results, market_name):
    ...
    for i, item in enumerate(top3):
        ...
        lines.append(
            ...
            f"   현재: {price:,}원 ({item['change_pct']:+.2f}%) | 필수 {s['mandatory']}/6 우대 {s['bonus']}/4\n"
...
def main():
    ...
    # 1. 거래량 TOP 보통주 수집 (한투 + 네이버 시장구분)
    print("\n[1/3] 거래량 순위 수집")
    kospi_cand, kosdaq_cand = get_volume_rank_all(count=200)
    ...
    print(f"  → 후보: {len(kospi_results)}개")

    print(f"  코스닥 {len(kosdaq_cand)}개 스캔...")
    kosdaq_results = scan_dante(kosdaq_cand, "코스닥")
    print(f"  → 후보: {len(kosdaq_results)}개")
...
🔵 코스피 TOP3 (단테 점수순)
{'─'*30}
{format_dante_top3(kospi_results, "코스피")}

🟢 코스닥 TOP3 (단테 점수순)
{'─'*30}
{format_dante_top3(kosdaq_results, "코스닥")}

📊 스캔: 코스피 {len(kospi_cand)}개 + 코스닥 {len(kosdaq_cand)}개
